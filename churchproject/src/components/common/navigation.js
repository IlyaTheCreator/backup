...
export default function Navigation () {
    ...
    useEffect(() => {
        window.addEventListener('mouseup', e => {
            if (e.target !== nav.current && e.target !== navBtn.current) {
                setNavOpened(false)
            }
        })
    }, [])
    ...
    return (
        <nav className="navigation">
            ...
            <ul ref={nav} className={navOpened ? 'active' : ''}>
            ...
            <i ref={navBtn} onClick={() => setNavOpened(!navOpened)} className="ionicons icon ion-ios-menu" />
        </nav>
    )
}